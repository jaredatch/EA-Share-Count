		...
		var opts = {
			...
			success: function(res){
				...
				if ( res.success == true ) {
					...
					$('#ea-share-count-metabox .inside').prepend(res.data.date).prepend(res.data.list).prepend('<p id="snapshot-msg" class="'+res.data.class+'">'+res.data.msg+'</p>');				
				} else {
					$('#ea-share-count-msg').remove();
					$('#ea-share-count-metabox .inside').prepend('<p id="snapshot-msg" class="'+res.data.class+'">'+res.data.msg+'</p>');			
				}
